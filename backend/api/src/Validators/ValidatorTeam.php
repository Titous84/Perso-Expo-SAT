    ...
    public function validate( array $team ) : Result
    {
        ...
        if(!isset($team['year'])){
            ...
            if($membersMax == null){
                ...
                // Permettre les équipes à un seul membre.
                // @author Nathan Reyes
                if($sizeofArray < 1){
                    $messages[] = "Il doit avoir minimum un membre dans l'équipe.";
                }
                ...
                for($a = 0; $a < $sizeofArray; $a++){
                    ...
                    // Validation stricte du DA à 7 chiffres.
                    // @author Nathan Reyes
                    else if (!preg_match('/^\d{7}$/', strval($team['members'][$a]['numero_da']))) {
                        $messages[] = "Le numéro DA doit contenir exactement 7 chiffres : " . $team['members'][$a]['numero_da'];
                    }
