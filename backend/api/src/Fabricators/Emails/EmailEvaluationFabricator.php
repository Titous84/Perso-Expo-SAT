...
/** 
 * Classe permettant d'envoyer un courriel contenant le lien afin d'accéder à une évaluation. 
 * @author Christopher Boisvert
 * @author Mathieu Sévégny
 * @package App\Fabricators\Emails
*/
class EmailEvaluationFabricator
{
    /**
	...
	/**
	 * Fonction invoquée lors de l'appelle de la classe EmailEvaluationFabricator.
	 * @author Mathieu Sévégny
	 ...
	 * @param string $first_name Prénom du juge.
	 * @param string $last_name Nom du juge.
	 * @param string $uuid UUID du juge.
	 */
    public function send_mail(string $email, string $firstName, string $lastName, string $uuid)
    {
		try
		{
			$html = $this->twigService->twig->render('Emails/EvaluationMail.html.twig', [
			...
			$emailJSON = [
				...
				"subject"=>"ExpoSAT - Vos évaluations à compléter",
				"text_content"=>null,
				"html_content" => $html
			];
