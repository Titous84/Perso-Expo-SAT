...
class UserRepository extends Repository
{
	 ...
	 * 
	 * Bugfix : Permet de supprimer un juge et toutes les données associées (évaluations, critères d'évaluation, etc.) sans supprimer les autres juges ou les administrateurs.
   ...
   public function delete_judge( int $users_id ): int
{
    try
    {
        // 1. Récupère l'id du juge à partir du users_id
        $sql = "SELECT id FROM judge WHERE users_id=:users_id;";
        $req = $this->db->prepare($sql);
        ...
        // 2. Supprime les entrées dans categories_judge
        $sql = "DELETE FROM categories_judge WHERE judge_id=:judge_id;";
        $req = $this->db->prepare($sql);
        $req->execute(array("judge_id" => $judge_id));

        // 3. Supprime les criteria_evaluation liés aux évaluations du juge
        ...
        // 4. Supprime les évaluations du juge
        $sql = "DELETE FROM evaluation WHERE judge_id=:judge_id;";
        $req = $this->db->prepare($sql);
        $req->execute(array("judge_id" => $judge_id));

        // 5. Supprime le juge
        $sql = "DELETE FROM judge WHERE id=:judge_id;";
        $req = $this->db->prepare($sql);
        $req->execute(array("judge_id" => $judge_id));

        // 6. Supprime l'utilisateur
        $sql = "DELETE FROM users WHERE id=:id;";
        $req = $this->db->prepare($sql);
        $req->execute(array("id" => $users_id));
