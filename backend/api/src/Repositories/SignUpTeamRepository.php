 ...
 * Permet d'accéder à la base de données pour l'inscription d'une équipe
...
{    
    ...
    public function add_team(Team $team, array $token):Result
    {
        try{
            ...
            // @author Nathan Reyes
            $team->team_number = $this->generate_next_team_number($categoryArray);
        ...
        for($a = 0; $a < $sizeofArray; $a++){
            try{
                ...
                if($memberVerification['id'] == null){
                    ...
                    $this->errorMessages[] = "Les membres n'ont pas été ajouté.";
                    return;
                }
                else{
...
/**
 * get_all_contact_persons
 * Retourne toutes les personnes ressources de la base de données.
 * @author Léandre Kanmegne - H26
 * @return array
 */
public function get_all_contact_persons(): array {
    try {
        $sql = "SELECT id, name, email FROM contact_person ORDER BY name";
        $req = $this->db->prepare($sql);
        $req->execute();
        return $req->fetchAll();
    } catch (PDOException $e) {
    ...
    {
        try{
            $sql = "SELECT id,survey_id,acronym FROM categories WHERE name = :category";
            $req = $this->db->prepare($sql);
            $req->execute(array(
                "category" => $category
            ));
    ...
    /**
     * Génère le prochain numéro d'équipe selon l'acronyme de catégorie.
     * @author Nathan Reyes
     * @param array $category Tableau contenant au minimum id et acronym de la catégorie.
     * @return string
     */
    private function generate_next_team_number(array $category): string
    {
        try {
            // Conserver l'acronyme tel que défini dans la catégorie (ex: Info, SH-IS, SB-P).
            // @author Nathan Reyes
            $acronym = trim($category["acronym"] ?? "GEN");
            $prefixLength = strlen($acronym);

            $sql = "SELECT MAX(CAST(SUBSTRING(team_number, :start_position) AS UNSIGNED)) AS max_number
                    FROM teams
                    WHERE categories_id = :category_id
                      AND team_number LIKE :prefix_like";

            $req = $this->db->prepare($sql);
            $req->execute([
                "start_position" => $prefixLength + 1,
                "category_id" => $category["id"],
                "prefix_like" => $acronym . "%"
            ]);

            $response = $req->fetch();
            $maxNumber = isset($response["max_number"]) ? intval($response["max_number"]) : 0;

            return $acronym . strval($maxNumber + 1);
        } catch (PDOException $e) {
            $context["http_error_code"] = $e->getCode();
            $this->logHandler->critical($e->getMessage(), $context);
            return "GEN1";
