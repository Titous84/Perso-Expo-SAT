...
class TeamsListRepository extends Repository
{
    ...
    */
    ...
    {
        try {
            $sql = "SELECT id, name, acronym, activated, survey_id FROM categories ORDER BY name";
    ...
    /**
     * Fonction qui récupère le nom d'une catégorie à partir de son ID
     * @param int $id L'ID de la catégorie
     * @return array Retourne un tableau contenant l'id et le nom de la catégorie ou un tableau vide.
     * @author Breno Gomes - H26
    */
    public function get_category_by_id(int $id): array
    {
        try {
            $sql = "SELECT id, name, acronym, activated, survey_id FROM categories WHERE id = :id";
    ...
    * @author Breno Gomes - H26
    */
   public function add_new_category(string $name, string $acronym): bool
    {
        try {
            $sql = "INSERT INTO categories (name, activated, max_members, survey_id, acronym)
                    VALUES (:name, :activated, :max_members, :survey_id, :acronym)";
    ...
    /**
     * Fonction qui permet de mettre à jour une catégorie
     * @param int $id L'ID de la catégorie
     * @param string $name Le nom de la catégorie
     * @param string $acronym L'acronyme de la catégorie
     * @return array Retourne un tableau vide si succès, sinon les erreurs
     * @author Morgan Boissonneault - H26
     */
    public function update_category(int $id, string $name, string $acronym, int $survey_id, int $activated): array
    {
        try {
            ...
            $context["http_error_code"] = $e->getCode();
            $this->logHandler->critical($e->getMessage(), $context);
            return ['error' => 'Échec de la mise à jour : modification invalide.'];
        }
    }
    ...
    /**
     * Fonction qui permet de supprimer plusieurs catégories
     * @param array $categories Un tableau contenant les ID des catégories à supprimer
     * @return array Retourne un tableau vide si succès, sinon les erreurs
     * Corrections apportées par l'IA
     * @author Breno Gomes - H26
     */
    public function delete_categories(array $categories): array
    {
        $errors = [];
        foreach ($categories as $id) {
            $result = $this->delete_categorie((int)$id);
            if (!$result) {
                $errors[] = $id;
            }
        }
     ...
     * @author Breno Gomes - H26
     */
    public function delete_categorie(int $id): bool
    {
        try {
            $this->db->beginTransaction();
            
            //Supprimer les références dans la table judge
            $sqlJudge = "DELETE FROM judge WHERE categories_id = :id";
            $reqJudge = $this->db->prepare($sqlJudge);
            $reqJudge->execute(["id" => $id]);
            
            //Supprimer les références dans la table teams
            $sqlTeams = "DELETE FROM teams WHERE categories_id = :id";
            $reqTeams = $this->db->prepare($sqlTeams);
            $reqTeams->execute(["id" => $id]);
            ...
            $context["http_error_code"] = $e->getCode();
            $this->logHandler->critical($e->getMessage(), $context);
            $this->errorMessages[] = "deleteCategorie: " . $e->getMessage();
            return false;
        }
    }
