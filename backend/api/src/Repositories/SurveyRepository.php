...
{    
    ...
    /**
     * Vérifie si un juge a au moins une équipe assignée.
     * @param int $judgeId L'id du juge dans la table judge.
     * @return bool True si le juge a au moins une évaluation active.
     * @author Léandre Kanmegne - H26
     */
    public function judge_has_active_evaluations(int $judgeId): bool
    {
        $sql = "SELECT COUNT(*) FROM evaluation 
                INNER JOIN judge ON evaluation.judge_id = judge.id
                WHERE judge.users_id = :judge_users_id AND evaluation.est_actif = 1";
        $req = $this->db->prepare($sql);
        $req->execute(["judge_users_id" => $judgeId]);
