    ...
    public function add_signup_team( array $teamJson ): Result
    {
        try
	    {
            ...
            for($a = 0; $a < $sizeofArray; $a++){
               ...
               $mailResult = $sendMail->send_mail_contact_person($team->contactPerson[$a]["fullName"], $team->contactPerson[$a]["email"], $team->members, $team->title, $team->description, $team->category, $team->year);

               // Arrêter le traitement et retourner une erreur claire si un courriel échoue.
               // @author Nathan Reyes
               if ($mailResult->get_http_code() !== EnumHttpCode::SUCCESS) {
                    return new Result(
                        EnumHttpCode::BAD_REQUEST,
                        array("Échec de l'envoi des courriels aux personnes-ressources."),
                        array($team->contactPerson[$a]["email"])
                    );
               }
            }
