...
class VerificationCodeService
{
	 ...
	 * Bugfix : le validateur était manquant, ce qui causait des erreurs lors de l'appel de la fonction generate_code.
	 * @author Léandre Kanmegne - H26
	 */
	private $validatorVerificationCode;
	/**
     * generatorUUID
     *
    ...
    public function verify_code(string $code): Result
    {
	    try
	    {
			...
			// Retourner un objet avec la clé "email" pour correspondre au contrat attendu par le front.
			// @author Nathan Reyes
			return new Result(EnumHttpCode::SUCCESS, array("Le code est valide"), array("email" => $verificationCodeResult));
	    }
		catch ( Exception $e )
		{
			$context["http_error_code"] = $e->getCode();
