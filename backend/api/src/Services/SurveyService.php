 ...
 * Code corrigé et amélioré par Léandre Kanmegne - H26, inspiré de la fonction send_all_survey_judge.
 * Verifie que les juges ne sont pas blacklisté et qu'ils ont au moins une équipe assignée, sinon affiche les erreurs. Ensuite, attribue des nouveaux UUID à chaque juge individuellement et envoie les courriels.
 */
public function send_all_survey_judgeIndividually(array $judges)
{
    try {
        ...
        // On va vérifier que les juges ne sont pas blacklisté et qu'ils ont au moins une équipe assignée, sinon on arrête le processus et on affiche les erreurs.
        $judgesSansEquipe = [];
        $judgesListeNoire = [];

        foreach ($judges as $judge) {
            if ($judge["blacklisted"] == true) {
                $judgesListeNoire[] = $judge["firstName"] . " " . $judge["lastName"];
            }
            if (!$this->surveyRepository->judge_has_active_evaluations($judge["id"])) {
                $judgesSansEquipe[] = $judge["firstName"] . " " . $judge["lastName"];
            }
        }

        $erreurs = [];

        if (count($judgesListeNoire) > 0) {
            $erreurs[] = "Dans la liste noire : " . implode(", ", $judgesListeNoire);
        }
        if (count($judgesSansEquipe) > 0) {
            $erreurs[] = "Sans équipe assignée : " . implode(", ", $judgesSansEquipe);
        }

        if (count($erreurs) > 0) {
            return new Result(EnumHttpCode::BAD_REQUEST, $erreurs);
        }
