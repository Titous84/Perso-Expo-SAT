...
class TeamsListService
{
     ...
     */
    public function add_category(string $name, string $acronym): Result {
        try {
            $response = $this->teamsListRepository->add_new_category($name, $acronym);

            if ($response === false) {
                return new Result(EnumHttpCode::BAD_REQUEST, array("Il n’y a eu aucune modification."));
            }

            return new Result(EnumHttpCode::SUCCESS, array('Success'), 'La catégorie a bien été ajoutée.');
        } catch (PDOException $e) {
            $context["http_error_code"] = $e->getCode();
            $this->logHandler->critical($e->getMessage(), $context);
     ...
     * @param string $name Le nom de la catégorie
     * @param string $acronym L'acronyme de la catégorie
     * @param int $survey_id L'id de la grille dans la BD
     * @return Result Retourne le résultat de l'opération
     * @author Morgan Boissonneault - H26
     */
    public function update_category(int $id, string $name, string $acronym, int $survey_id, int $activated): Result {
        try {
            $category = $this->teamsListRepository->get_category_by_id($id);
    
            if (count($category) === 0) {
                return new Result(EnumHttpCode::NOT_FOUND, ["La catégorie n'existe pas."]);
            }

            $response = $this->teamsListRepository->update_category($id, $name, $acronym, $survey_id, $activated);

            if (isset($response['error'])) {
                return new Result(EnumHttpCode::BAD_REQUEST, [$response['error']]);
            }

            return new Result(EnumHttpCode::SUCCESS, ['Success'], 'La catégorie a bien été mise à jour.');
        } catch (PDOException $e) {
            $context["http_error_code"] = $e->getCode();
            $this->logHandler->critical($e->getMessage(), $context);
            return new Result(EnumHttpCode::BAD_REQUEST, ["Une erreur est survenu lors de l'enregistrement des données."], $e);
        } catch (Exception $e) {
            $context["http_error_code"] = $e->getCode();
            $this->logHandler->critical($e->getMessage(), $context);
            return new Result(EnumHttpCode::SERVER_ERROR, ["Une erreur inattendue est survenue."]);
        }
    }
    ...
    /**
     * Fonction qui permet de supprimer une ou plusieurs catégories
   ...
   public function delete_categories(array $category): Result
    {
        try {
            ...
            return new Result(EnumHttpCode::BAD_REQUEST, array('Une erreur est survenue lors de la suppression des catégories.'));
        } catch (PDOException $e) {
            $context["http_error_code"] = $e->getCode();
            $this->logHandler->critical($e->getMessage(), $context);
            return new Result(EnumHttpCode::BAD_REQUEST, array("Une erreur est survenue lors de la suppression des catégories"));
