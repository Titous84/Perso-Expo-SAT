...
export default class UserService {
    ...
    public static async ChangePwUser(email:string,pwd:string) : Promise<APIResult<string[]>>{
        ...
        // Route utilisée par le parcours "mot de passe oublié" sans session active.
        // @author Nathan Reyes
        const response : APIResult<string[]> = await APIRequest(`user/change-pwd`,"PATCH",false,body)
        return response;
    }
    ...
    public static async PasswordForgoten(email:string,verificationCode:string) : Promise<APIResult<string[]>>{
        ...
        // Route publique de demande de réinitialisation du mot de passe.
        // @author Nathan Reyes
        const response : APIResult<string[]> = await APIRequest(`user/password-email`,"POST",false,body)
        return response;
    }
}
