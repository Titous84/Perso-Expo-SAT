...
interface TeamContactPersonProps {
  ...
  onChangeContactPerson: (number: number) => void;
  readOnly: boolean;
}
...
export default class TeamContactPerson extends React.Component<TeamContactPersonProps> {
  componentDidMount() {
    ValidatorForm.addValidationRule('maxLenghtFullName', (value) => {
      if (value.length > MAX_LENGTH_CONTACT_PERSON_NAME) return false;
      return true;
    });
      ...
      >
        <Paper className={`${styles.subhead} ${styles.stack}`}>
          <div className={styles.flexTitle}>
            <h2>
              {TEXTS.signup.information.contactPerson.label1}
              {this.props.number === 1 ? (
                <>{TEXTS.signup.information.contactPerson.label2}</>
              ) : (
                <>
                  {TEXTS.signup.information.contactPerson.label3}{' '}
                  {this.props.number}
                  <sup>{'e'} </sup>
                </>
              )}
        ...
        <Grid2 container spacing={2}>
          <Grid2 size={{ xs: 12, md: 12 }}>
            <TextValidator
              ...
              onChange={(event: any) =>
                this.props.handleChangeContact(
                  this.props.number,
                  'fullName',
                  event.target.value,
                )
              ...
              errorMessages={[
                TEXTS.signup.information.contactPerson.firtsLastName.error
                  .required,
                TEXTS.signup.information.contactPerson.firtsLastName.error
                  .maximum,
              ]}
              inputProps={{
                maxLength: MAX_LENGTH_CONTACT_PERSON_NAME,
                readOnly: this.props.readOnly,
              }}
              fullWidth
            />
            <p className={styles.alignRight}>
              {this.props.contactPerson.fullName.length} /{' '}
              {MAX_LENGTH_CONTACT_PERSON_NAME}
            </p>
          </Grid2>
          <Grid2 size={{ xs: 12, md: 12 }}>
            <TextValidator
              ...
              validators={[
                'required',
                'matchRegexp:' + REGEX.CONTACT_PERSON,
                'maxLenghtEmail',
              ]}
              errorMessages={[
                TEXTS.signup.email.error.required,
                TEXTS.signup.email.errorContactPerson.invalide,
                TEXTS.signup.email.error.maximum,
              ]}
              inputProps={{
            ...
            />
            <p className={styles.alignRight}>
              {this.props.contactPerson.email.length} / {MAX_LENGTH_EMAIL}
            </p>
          </Grid2>
          {/* Bouton de modification — @author Léandre Kanmegne - H26 */}
          <Grid2 size={{ xs: 12, md: 12 }} className={styles.center}>
            <Button
              variant={INPUT_VARIANT}
              className={styles.boutonMembre}
              onClick={() =>
                this.props.onChangeContactPerson(this.props.number)
              }
            >
              {TEXTS.signup.information.buttonContactPerson.modify}
            </Button>
          </Grid2>
        </Grid2>
      </Paper>
