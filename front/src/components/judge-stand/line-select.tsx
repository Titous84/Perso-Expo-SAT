...
> {
  ...
  CheckEvals() {
    this.props.standsEval.map((evals) => {
      // Bugfix: compare avec le vrai time_slots.id
      // @author Léandre Kanmegne - H26
      const timeSlotId = this.props.hours[this.props.placement]?.id;
      if (evals.hour !== timeSlotId) return;

      if (this.props.leJuge.id !== evals.judge_id) return;
  ...
  updateOrAddSurvey(standId: string, surveyId: number) {
    ...
    // Bugfix: utilise le vrai time_slots.id au lieu de l'index de colonne
    // @author Léandre Kanmegne - H26
    const timeSlotId = this.props.hours[this.props.placement].id;
  ...
  handleChange(e: SelectChangeEvent<string>) {
    ...
    this.props.stands.map((stand) => {
      if (stand.team_number != e.target.value) return;
  ...
  sortStand(): StandInfo[] {
    return this.props.stands.sort((stand1, stand2) => {
      if (
        this.props.leJuge.categories_id !== stand1.categories_id &&
        this.props.leJuge.categories_id === stand2.categories_id
      ) {
        return 1;
      }
    ...
    return (
      <FormControl sx={{ m: 1, minWidth: 75 }}>
        ...
        <Select
          ...
          sx={{
            backgroundColor: this.props.verifyIfTeamIsAssignedMoreThan3Times(
              this.props.standsEval,
              this.state.standId,
            )
              ? 'yellow'
              : '',
