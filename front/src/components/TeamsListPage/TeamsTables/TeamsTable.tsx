...
export default function TeamsTable() {
  ...
  const [snackbarMessageType, setSnackbarMessageType] =
    useState<SnackbarMessageType>('error'); // Type de message à afficher dans le snackbar. Peut être "success", "error", "warning" ou "info".
  // Fenêtre contextuelle de confirmation pour la suppression des équipes sélectionnées.
  const [isConfirmationDialogOpen, setIsConfirmationDialogOpen] =
    useState(false);

  // Détail d'une équipe pour afficher toutes les informations sans troncature.
  // @author Nathan Reyes
  ...
  const formatMembersForDisplay = (members: ITeam['members']) => {
    if (Array.isArray(members)) {
      ...
      if (typeof members[0] === 'string') return (members as unknown as string[]).join(', ');
      return members
        .map((member: any) =>
  ...
  const processRowUpdate = async (updatedRow: ITeam) => {
    try {
      ...
      const originalRow = teamsArray.find(
        (team) => team.team_id === updatedRow.team_id,
      );
      ...
      setTeamsArray((prevTeams) =>
        prevTeams.map((team) =>
          team.team_id === rowToSend.team_id ? rowToSend : team,
        ),
      );
    ...
    TeamsListService.deletesTeamsInfos(selectedTeamsIds)
      .then((response) => {
        if (response.error) {
          ...
          setTeamsArray((prevTeams) =>
            prevTeams.filter((team) => !selectedTeamsIds.includes(team.team_id)),
          );
        }
      })
      .catch((error) => {
  ...
  return (
    <>
      ...
      <ConfirmationDialog
        ...
        content={
          'Êtes-vous sûr de vouloir supprimer les équipes sélectionnées?'
        } // Passer un contenu à afficher dans la fenêtre contextuelle de confirmation.
      ...
      {/* Fenêtre de détails pour voir toutes les informations de l'équipe sélectionnée. */}
      {/* @author Nathan Reyes */}
      <Dialog
      ...
      >
        ...
        <DialogContent dividers>
          {teamDetail && (
            <Stack spacing={1.2}>
              <Typography>
                <strong>Numéro d'équipe :</strong> {teamDetail.team_number}
              </Typography>
              <Typography>
                <strong>Titre :</strong> {teamDetail.title}
              </Typography>
              <Typography>
                <strong>Description :</strong> {teamDetail.description}
              </Typography>
              <Typography>
                <strong>Année :</strong> {teamDetail.year}
              </Typography>
              <Typography>
                <strong>Catégorie :</strong> {teamDetail.category}
              </Typography>
              <Typography>
                <strong>Type d'évaluation :</strong> {teamDetail.survey}
              </Typography>
              <Typography>
                <strong>Équipe activée :</strong>{' '}
                {teamDetail.teams_activated === 1 ? 'Oui' : 'Non'}
              </Typography>
              <Typography>
                <strong>Membres :</strong>{' '}
                {formatMembersForDisplay(teamDetail.members)}
              </Typography>
              <Typography>
                <strong>Nom de l'enseignant(e) :</strong>{' '}
                {teamDetail.contact_person_name}
              </Typography>
              <Typography>
                <strong>Courriel de l'enseignant(e) :</strong>{' '}
                {teamDetail.contact_person_email}
              </Typography>
            </Stack>
          )}
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setTeamDetail(null)}>Fermer</Button>
        </DialogActions>
      </Dialog>
      ...
      <Box sx={{ width: '100%', overflowX: 'auto' }}>
        <DataGrid<ITeam>
          ...
          slots={{
            ...
            toolbar: (props) => (
              // `toolbar` fourni un `props` qui contient des données comme les rangées du tableau sélectionnées, etc.
              <TeamsTableToolbar
                {...props}
