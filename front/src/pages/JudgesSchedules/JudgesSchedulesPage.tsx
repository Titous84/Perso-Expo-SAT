...
> {
   ...
   * Correction de la fonction loadStands pour gérer correctement les erreurs en vérifiant à la fois la présence d'une erreur et l'existence des données dans la réponse de l'API, ce qui permet d'afficher une erreur appropriée en cas de problème de communication ou de données manquantes.
  ...
  async loadStands() {
    ...
    if (response.error) {
      ShowToast(
        "une erreur est survenue veuillez contacter l'administration",
        5000,
        'error',
        'top-center',
        false,
   ...
   * Ajout d'un toast de succès ou d'erreur selon la réponse de l'API
   * @author Léandre Kanmegne H-26
   */
  async UpdateSurvey(evalId: number, stand_id: string, survey_id: number) {
    const response = await JugeStandService.PatchStandSurvey(
      evalId,
      stand_id,
      survey_id,
    );
    if (response.error) {
      ...
      ShowToast('Assignation modifiée', 3000, 'success', 'top-center', false);
    }
  }
