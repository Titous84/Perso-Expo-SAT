...
describe('AdministratorsListPage', () => {
  ...
  test('déclenche la réinitialisation annuelle et affiche un succès', async () => {
    ...
    const confirmationButton = await screen.findByRole('button', { name: /confirmer la réinitialisation/i });
    expect(confirmationButton).toBeDisabled();

    await userEvent.type(
      screen.getByLabelText(/tapez « réinitialiser » pour confirmer/i),
      'RÉINITIALISER'
    );
  ...
  test('n’autorise pas la confirmation tant que le mot-clé est incorrect', async () => {
    render(<AdministratorsListPage />, { wrapper: MemoryRouter });
    ...
    expect(confirmationButton).toBeDisabled();
    expect(UserService.resetEventData).not.toHaveBeenCalled();
  });
});
